...
class PrivacyPassRedeemerTest < ActiveSupport::TestCase
  ...
  context '#redeem' do
    ...
    should 'return false when token type is not 2 (publicly verifiable RSA Blind Signatures)' do
      ...
      new_authenticator = generate_authenticator(@rsa_keypair, invalid_token_type, @nonce, @challenge_digest,
                                                 @token_key_id)
    ...
    should 'return false when the token is not the correct length' do
      ...
      new_authenticator = generate_authenticator(@rsa_keypair, @token_type, invalid_nonce, @challenge_digest,
                                                 @token_key_id)
    ...
    should "return false when the token cannot be verified with the issuer's public key" do
      ...
      new_authenticator = generate_authenticator(@rsa_keypair, @token_type, @nonce, unregistered_challenge_digest,
                                                 @token_key_id)
    ...
    should "return true when the token can be verified with the issuer's public key" do
      ...
      new_authenticator = generate_authenticator(@rsa_keypair, @token_type, @nonce, registered_challenge_digest,
                                                 @token_key_id)
